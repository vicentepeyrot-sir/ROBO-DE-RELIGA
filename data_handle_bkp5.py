...
class DataHandling:
    ...
    def data_processing(self, variante):
        ...
        try:
            ...
            try:
                sap_concat['num_grupo_retorno_campo'] = sap_concat['Grupo Retorno de Campo'].astype(str).str.replace(
                    r'\D+', '', regex=True
                )

                sap_remove = sap_concat[[
                    'Nota', 'num_grupo_retorno_campo', 'prazo_SAP', 'prazo_GPM'
                ]].copy()

                sap_remove = sap_remove.dropna(subset=['num_grupo_retorno_campo'])
                sap_remove = sap_remove[
                    ~sap_remove['num_grupo_retorno_campo'].isin(['600', '9000'])
                ]
