...
class DataHandling:
    ...
    def create_nimport_file(self):
        ...
        nimport_df = pd.read_csv(
            'https://docs.google.com/spreadsheets/d/e/2PACX-1vSdYjH0obp2tqFvQS6qnA5eGiava-HJIM_Z1A5RRNqpn0O8nsyUtKPeKilemyf60IgC8RrnQFG-B5ru/pub?gid=1341271327&single=true&output=csv', sep=",", encoding='utf-8', skip_blank_lines=True, header=2, dtype=str)

        print(repr(nimport_df.columns.tolist()))

        nimport_df = nimport_df.loc[:, ~
                                    nimport_df.columns.str.contains('Unnamed')]
        ...
        try:
            nimport_df['Nota'] = nimport_df['Nota'].astype(
                np.int64).astype(str)
        except:
            nimport_df['Nota'] = nimport_df['Nota'].astype(str)
