...
class DataHandling:
    ...
    def create_nimport_file(self):
        ...
        # Descobre automaticamente qual linha é o cabeçalho (onde aparece "Nota")
        preview = pd.read_csv(url, sep=",", encoding="utf-8", header=None, nrows=20, dtype=str)
        ...
        if header_row is None:
            raise ValueError("Não encontrei a coluna 'Nota' nas primeiras linhas do CSV do nimport.")
        ...
        # Limpa colunas "Unnamed" e vazias
        nimport_df.columns = [str(c).strip() for c in nimport_df.columns]
        nimport_df = nimport_df.loc[:, ~nimport_df.columns.str.contains('^Unnamed', case=False, regex=True)]
        nimport_df = nimport_df.dropna(axis=1, how='all').dropna(axis=0, how='all')

        # Garante a coluna Nota
        if "Nota" not in nimport_df.columns:
            cols_upper = {c.upper(): c for c in nimport_df.columns}
            if "NOTA" in cols_upper:
                nimport_df.rename(columns={cols_upper["NOTA"]: "Nota"}, inplace=True)
            else:
                raise KeyError(f"Coluna 'Nota' não encontrada. Colunas atuais: {nimport_df.columns.tolist()}")

        nimport_df = nimport_df[nimport_df["Nota"].notna()].reset_index(drop=True)
        ...
        # Salva no caminho correto (use o self.file_nimport_csv que você já tem)
        nimport_df.to_csv(
    ...
    def data_processing(self, variante):
        ...
        try:
            ...
            sap_concat['prazo_GPM'] = pd.to_datetime(
                sap_concat['prazo_servico'], dayfirst=True)
            sap_concat['prazo_SAP'] = pd.to_datetime(sap_concat['concl_desejada'].astype(str) + ' ' + sap_concat['H final desej.'].astype(str),
                                                     errors='coerce')

            sap_concat['prazo_SAP'] = sap_concat['prazo_SAP'].map(lambda x: x.replace(second=0))
            # sap_concat = sap_concat.query('prazo_SAP != "" & prazo_GPM == "" | prazo_SAP > prazo_GPM')
                           
            # ALTERAÇÃO AQUI: Adicionado .copy() ao final
            sap_concat = sap_concat.query('prazo_SAP != "" & prazo_GPM == "" | prazo_SAP > prazo_GPM').copy()


            # filtrar e remover as notas que o retorno de campo são diferentes de 600 ou 9000
            print('#Removendo notas com retorno de concluidas e finalizadas')
            try:
                sap_concat['num_grupo_retorno_campo'] = sap_concat['Grupo Retorno de Campo'].str.replace('\D+',
                                                                                                         '',
                                                                                                         regex=True)
                sap_remove = sap_concat[[
                    'Nota', 'num_grupo_retorno_campo', 'prazo_SAP', 'prazo_GPM']].copy()
                sap_remove = sap_remove.query(
                    'num_grupo_retorno_campo != 600 | num_grupo_retorno_campo != 9000')
                sap_remove = sap_remove.dropna()
                sap_concat = sap_concat[~sap_concat['num_grupo_retorno_campo'].isin(
                    sap_remove['num_grupo_retorno_campo'])]
            ...
            for local_do_corte in dict_local_corte:
                sap_concat.loc[(sap_concat['Tipo de nota iw59'] == 'CS') & (sap_concat['Nº do pedido'].astype(str).str.contains(
                    local_do_corte)), 'local_corte'] = 'Local Corte: ' + dict_local_corte[local_do_corte] + ' | '

            if 'local_corte' in sap_concat.columns:
                sap_concat['nova_desc'] = sap_concat['local_corte'].astype(
                    str).str.cat(sap_concat['Descrição'], sep='')
                sap_concat['nova_desc'] = sap_concat['nova_desc'].astype(
                    str).str.replace('nan', '', regex=True)
