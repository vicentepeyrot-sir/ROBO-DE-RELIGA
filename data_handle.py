...
class DataHandling:
    ...
    def copiar_notas_a_importar(self):
        ...
        gpm_df['Nota'] = self._normalizar_nota(gpm_df['Nota'])
        ...
        iw59_df['Nota'] = self._normalizar_nota(iw59_df['Nota'])
        ...
        nimport_df = pd.read_csv(self.file_nimport_csv, sep=";",
                                 on_bad_lines='skip',
                                 encoding='utf8')
        nimport_df['Nota'] = self._normalizar_nota(nimport_df['Nota'])
        ...
        # remove notas que já existem no GPM para evitar reimportação/duplicidade
        print('#Removendo notas já existentes no GPM')
        notas_existentes_gpm = set(gpm_df['Nota'].dropna())
        notas_existentes_gpm.discard('')
        iw59_df = iw59_df[~iw59_df['Nota'].isin(notas_existentes_gpm)]
        ...
        # Deduplica por Nota antes de copiar para evitar notas repetidas no clipboard
        notas_unicas = iw59_df['Nota'].drop_duplicates().tolist()
        pyperclip.copy("\r\n".join(notas_unicas))
        sleep(1)
    ...
    def data_processing(self, variante):
        ...
        try:
            ...
            txt_df['Nota'] = self._normalizar_nota(txt_df['Nota'])
            ...
            # Garante que txt_df não tenha duplicatas por Nota antes do merge
            txt_df_dedup = txt_df[['Nota', 'Texto']].drop_duplicates(subset=['Nota'], keep='first')
            sap_concat = pd.merge(iw59_df, txt_df_dedup,
                                  how='left',
                                  on='Nota')

            # Traz a as notas abertas e concluidas para o arquivo da IW59 que já esta com as observações e remove as notas abertas
            print('#Removendo notas abertas da importação')
            # Garante que gpm_abertas não tenha duplicatas por Nota antes do merge
            gpm_abertas_dedup = gpm_abertas.drop_duplicates(subset=['Nota'], keep='first')
            sap_concat = pd.merge(sap_concat,
                                  gpm_abertas_dedup,
                                  how='left',
                                  on='Nota')
            sap_concat = sap_concat.query('situacao_servico != "Aberto"')
            sap_concat.drop_duplicates(subset=['Nota'], inplace=True, keep='first')
            ...
            coords_df = pd.read_csv(self.file_coords_csv, sep=";",
                                    on_bad_lines='skip',
                                    encoding='utf8')
            coords_df['Nota'] = self._normalizar_nota(coords_df['Nota'])
            ...
            # Remove do sap_concat_3 as notas que já têm coordenadas em sap_concat
            # para evitar duplicidade ao concatenar
            notas_com_coords = sap_concat[sap_concat['Latitude'].notna()]['Nota']
            sap_concat_3 = sap_concat_3[~sap_concat_3['Nota'].isin(notas_com_coords)]
            sap_concat = pd.concat([sap_concat,
                                    sap_concat_3])
            sap_concat = sap_concat.drop_duplicates(subset=['Nota'], keep='first')
            ...
            # Deduplica cortes_df por Nota antes do merge para evitar fan-out de linhas
            cortes_df_dedup = cortes_df[['Nota', 'Valores', 'code_cortes']].drop_duplicates(subset=['Nota'], keep='first')
            sap_concat = sap_concat.merge(
                cortes_df_dedup, how='left', on='Nota')
            ...
            # Deduplica por data antes do merge para evitar multiplicação de linhas
            # se a planilha de Feriados tiver datas repetidas
            df_feriados_dedup = df_feriados.drop_duplicates(subset=['data_feriado'], keep='first')
            df_vespera_dedup = df_vespera.drop_duplicates(subset=['data_vespera'], keep='first')
            # Mescla com o dataframe de feriado
            sap_concat = pd.merge(sap_concat, df_feriados_dedup,
                                  left_on='Data da nota', right_on='data_feriado', how='left')
            # Mescla com o dataframe de vespera
            sap_concat = pd.merge(sap_concat, df_vespera_dedup,
            ...
            # Usa o prazo calculado (que já respeita feriados/fins de semana) como prioridade.
            # Só preenche com prazo_SAP onde Prazo for NaN (ex: notas que não são religações).
            # Não usamos min() pois ele retornaria o prazo_SAP quando há feriado,
            # anulando o ajuste feito por calcular_prazo_religacoes.
            sap_concat['Prazo'] = sap_concat['Prazo'].combine_first(sap_concat['prazo_SAP'])
            ...
            # sap_com_prazo: apenas notas CR com Prazo válido (terão data ajustada).
            # sap_sem_prazo: todas as demais — complemento exato de sap_com_prazo.
            # Os dois conjuntos são mutuamente exclusivos para evitar duplicatas no concat.
            mask_com_prazo = sap_concat['Prazo'].notna() & sap_concat['Tipo de nota iw59'].str.contains("CR")
            sap_com_prazo = sap_concat[mask_com_prazo].copy()
            sap_sem_prazo = sap_concat[~mask_com_prazo].copy()
            ...
            importar_df['Nota'] = self._normalizar_nota(importar_df['Nota'])
    ...
    def _inicio_contagem(self, datahora):
        """
        Retorna o momento em que a contagem do prazo deve iniciar.

        Regras:
        - Dia útil (seg–sex, não feriado) entre 08:00 e 17:59 → contagem começa imediatamente.
        - Qualquer outro caso (após 18h, antes das 08h, sábado, domingo, feriado, véspera após 18h)
          → contagem começa às 08:00 do próximo dia útil, pulando feriados e fins de semana.
        """
        if self.is_dia_util(datahora) and 8 <= datahora.hour < 18:
            return datahora

        # Determina o candidato inicial às 08:00
        if datahora.hour >= 18:
            # Após 18h: começa no dia seguinte às 08h
            candidato = (datahora + pd.Timedelta(days=1)).replace(hour=8, minute=0, second=0)
        else:
            # Antes das 08h, fim de semana ou feriado durante o dia: tenta o mesmo dia às 08h
            candidato = datahora.replace(hour=8, minute=0, second=0)

        # Avança até encontrar um dia útil
        while not self.is_dia_util(candidato):
            candidato += pd.Timedelta(days=1)

        return candidato.replace(hour=8, minute=0, second=0)

    def calcular_prazo_religacoes(self, row):
        """
        Calcula o prazo de atendimento para notas de religação.

        Regras de negócio:
        - Urbana: prazo de 24h corridas a partir do início da contagem.
        - Rural:  prazo de 48h corridas a partir do início da contagem.

        O início da contagem é determinado por _inicio_contagem:
        - Dia útil entre 08h e 17:59 → conta da hora da geração.
        - Qualquer outro caso → conta a partir das 08h do próximo dia útil.
        """
        if 'Religação' not in row['Txt.code codif.']:
            return None

        horas_prazo = 48 if row['Urbano/Rural'] == 'R' else 24
        inicio = self._inicio_contagem(row['Gerado'])
        return inicio + pd.Timedelta(hours=horas_prazo)
