...
class GDrive():
    ...
    def __init__(self, account_type: str, _dir: Directories):
        self.account_type = account_type
        self.drive = None
        self._dir = _dir

        if os.path.isfile(os.path.join((self._dir.dir_data), 'chaveGoogle')):
            if account_type == "service_account":
                GDrive.drive_auth_service_account(
                    self, dir_data=self._dir.dir_data)
            elif account_type == "google_account":
                GDrive.drive_auth_google_account(self)
            else:
                raise ValueError("Login account type not supported")
        else:
            raise FileNotFoundError("Google account key not found")
    ...
    def drive_auth_service_account(self, dir_data):
        ...
        google_key = Decrypt(self._dir).decrypt_files('chaveGoogle')
            ...
            if len(file_list) > 0:
                for file1 in file_list:
                    if file1['title'] == title:
                        ...
                        file_uploaded = True
                        break
            if file_uploaded is False:
                self._upload_file(file, title, folder_id, True)
        except Exception as _e:  # pylint: disable=broad-except
            print(_e)
