...
class SapHandler:
    def fechar_aviso_licenca(self, session):
        try:
            # Verifica se existe uma janela de popup (geralmente index 1)
            if session.Children.Count > 1:
                # O botão de confirmação em popups de informação geralmente é o id "btn[0]"
                session.findById("wnd[1]/tbar[0]/btn[0]").press()
                print("Aviso de licença fechado com sucesso.")
        except Exception as e:
            # Se não encontrar a janela, apenas ignora
            pass
