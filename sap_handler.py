...
class SapHandler:
    ...
    def handle_session(self, session):
        try:
            ...
            elif session.children(1).text == "Informação":
                self.handle_license_info(session)
            else:
    ...
    def handle_license_info(self, session):
        try:
            print("Fechando aviso de Informação (Licença)...")
            session.findById("wnd[1]").sendVKey(0)
            sleep(1)
            # Se houver outro aviso em sequência (como logon múltiplo), trata ele também
            if session.children.count > 1:
                self.handle_session(session)
        except Exception as ex:
            print(f"Cannot close license info. Exit. {ex}")
            self.err_screenshot()
            raise RuntimeError("Cannot close license info. Exit.")
