...
def normalizar_identificador(valor):
    ...
    # Remove sufixo decimal de valores inteiros vindos como float no CSV/Excel
...
class DataHandling:
    ...
    def copiar_notas_a_importar(self) :
            ...
            iw59_df['prazo_GPM'] = pd.to_datetime(
                iw59_df['prazo_servico'], dayfirst=True, errors='coerce')

            prazo_sap_iw59 = (
                iw59_df['concl_desejada'].fillna('').astype(str).str.strip() + ' ' +
                iw59_df['H final desej.'].fillna('').astype(str).str.strip()
            ).str.strip()
            ...
            iw59_df.loc[iw59_df['prazo_SAP'].notna(), 'prazo_SAP'] = iw59_df.loc[
                iw59_df['prazo_SAP'].notna(), 'prazo_SAP'
    ...
    def create_nimport_file(self):
        ...
        url = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSdYjH0obp2tqFvQS6qnA5eGiava-HJIM_Z1A5RRNqpn0O8nsyUtKPeKilemyf60IgC8RrnQFG-B5ru/pub?gid=1341271327&single=true&output=csv'

        # Lê várias linhas sem cabeçalho para descobrir onde está a linha real do cabeçalho
        preview = pd.read_csv(
            url,
            sep=",",
            encoding="utf-8",
        ...
        for i in range(len(preview)):
            row = preview.iloc[i].astype(str).str.strip().str.upper().tolist()
            if "NOTA" in row:
                header_row = i
                break

        if header_row is None:
            print("Preview das primeiras linhas do CSV:")
            print(preview.head(10).to_string())
            raise ValueError("Não encontrei a coluna 'Nota' nas primeiras 50 linhas do CSV do nimport.")

        # Lê novamente usando a linha correta como cabeçalho
        nimport_df = pd.read_csv(
            url,
            sep=",",
            encoding="utf-8",
        ...
        print("Colunas encontradas no nimport:")
        print(repr(nimport_df.columns.tolist()))

        # Remove colunas Unnamed e colunas totalmente vazias
        nimport_df = nimport_df.loc[:, ~nimport_df.columns.str.contains(r'^Unnamed', case=False, regex=True)]
        nimport_df = nimport_df.dropna(axis=1, how='all')
        nimport_df = nimport_df.dropna(axis=0, how='all')

        # Procura a coluna Nota de forma segura
        col_nota = None
        for col in nimport_df.columns:
            if str(col).strip().upper() == "NOTA":
                col_nota = col
                break

        if col_nota is None:
            raise KeyError(f"Coluna 'Nota' não encontrada. Colunas atuais: {nimport_df.columns.tolist()}")

        if col_nota != "Nota":
            nimport_df.rename(columns={col_nota: "Nota"}, inplace=True)

        # Limpa e filtra a coluna Nota
        nimport_df["Nota"] = nimport_df["Nota"].astype(str).str.strip()
    ...
    def data_processing(self, variante):
        ...
        try:
            ...
            cortes_df = txt_df[(txt_df['Tipo da Nota'].str.contains('CS')) | (
                txt_df['Tipo da Nota'].str.contains('CA'))]
            ...
            # monta o prazo do SAP com formato explícito para evitar warning e parsing inconsistente
            prazo_sap_str = (
                sap_concat['concl_desejada'].fillna('').astype(str).str.strip() + ' ' +
                sap_concat['H final desej.'].fillna('').astype(str).str.strip()
            ).str.strip()
            ...
            sap_concat.loc[sap_concat['prazo_SAP'].notna(), 'prazo_SAP'] = sap_concat.loc[
                sap_concat['prazo_SAP'].notna(), 'prazo_SAP'
            ].apply(lambda x: x.replace(second=0))

            print(f'#Qtd antes do filtro de prazo: {len(sap_concat)}')

            # mantém somente notas onde:
            # - existe prazo SAP e não existe prazo GPM; ou
            # - existe prazo SAP maior que o prazo GPM
            ...
            print(f'#Qtd após filtro de prazo: {len(sap_concat)}')
            ...
            try:
                ...
                print('#Códigos de retorno encontrados:',
                      sorted([x for x in sap_concat['num_grupo_retorno_campo'].dropna().astype(str).unique().tolist() if x != '']))
                ...
                print(f'#Qtd notas removidas por retorno de campo: {len(sap_remove)}')

                sap_concat = sap_concat[
                    ~sap_concat['Nota'].isin(sap_remove['Nota'])
                ].copy()

                print(f'#Qtd após filtro de retorno: {len(sap_concat)}')

            except Exception as e:
                print(f'#Sem notas com esse tipo de retorno ou erro no filtro: {e}')

            if sap_concat.empty :
                print('#sap_concat ficou vazio após os filtros')
                self.telegram.bot_message("Sem notas para importação")
                raise RuntimeError("There are no changes to do")
            ...
            if 'Instal' in bd_coord_df.columns:
                bd_coord_df['Instal'] = bd_coord_df['Instal'].apply(normalizar_identificador)
            ...
            if 'Instalação_x' in sap_concat.columns:
                sap_concat['Instalação_x'] = sap_concat['Instalação_x'].apply(normalizar_identificador)
            ...
            # se não tiver @D nem @S, mas tiver @P, usa Poste
            sap_concat.loc[
                mask_cs
                & ~pedido_str.str.contains('@D', na=False)
                & ~pedido_str.str.contains('@S', na=False)
            ...
            if 'Instalação' in sap_concat.columns:
                sap_concat['Instalação'] = sap_concat['Instalação'].apply(normalizar_identificador)
