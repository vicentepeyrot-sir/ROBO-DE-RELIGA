...
class DataHandling:
    ...
    def calcular_prazo_religacoes(self, row):
        ...
        if 'Religação' in row['Txt.code codif.']:
            is_feriado = row['feriado'] == 'S'
            is_vespera = row['vespera'] == 'S'
            hora_gerado = row['Gerado'].hour

            # ---------- FERIADOS E VÉSPERAS (regra especial, hora fixa às 08:00) ----------
            # Véspera a partir das 18h: prazo com +1 dia extra por causa do feriado seguinte
            if is_vespera and 18 <= hora_gerado <= 23:
            ...
            # Feriado ou véspera (demais casos): determina prazo com hora fixa às 08:00
            if is_feriado or is_vespera:
                # Feriado numa sexta → prazo na terça (+4d urbano / +5d rural)
            ...
            # ---------- DIAS NORMAIS (usa hora do SAP, sem travar às 08:00) ----------
            prazo_sap = row.get('prazo_SAP')

            # Dia de semana normal (seg–sex), entre 08:00 e 17:59 → usa horário do SAP
            if 0 <= row['Gerado'].weekday() <= 4 and 8 <= hora_gerado < 18:
                if row['Urbano/Rural'] == 'R':
                    return prazo_sap + pd.Timedelta(days=2) if pd.notna(prazo_sap) else row['Gerado'] + pd.Timedelta(days=2)
                else:
                    return prazo_sap + pd.Timedelta(days=1) if pd.notna(prazo_sap) else row['Gerado'] + pd.Timedelta(days=1)

            # Para todos os outros casos (após 18h, fins de semana, madrugada),
            # usa proximo_dia_util para fixar às 08:00 do próximo dia útil e soma os dias de prazo
